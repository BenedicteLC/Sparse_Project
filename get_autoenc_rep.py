...
def train():
	...
	if 5 != len(sys.argv):
	    print "Usage: python run_stacked_autoencoders_nnet.py lr hidden_size n_epochs noise_prob seed"
	    print ""
	    print "Ex.: python run_stacked_autoencoders_nnet.py 0.01 50 10 0.3 1234"
	    sys.exit()

	# Set the constructor
	str_ParamOption = "lr=" + sys.argv[0] + ", " + "hidden_size=" + sys.argv[1] + ", " + "n_epochs=" + sys.argv[2] + ", " \
		+ "noise_prob=" + sys.argv[3] + ", " + "seed=" + sys.argv[4]
	try:
	    objectString = 'myObject = Autoencoder(' + str_ParamOption + ')'
	    exec objectString
	...
	pickle.dump((myObject.W, myObject.b, myObject.hidden_size), open("Models/AutoE/model%d.pkl"%experiment_number, 'wb'))

def get_representation():
	# Load the dictionary and corresponding args.
	(W, b, hidden_size) = pickle.load(open("Models/AutoE/model%d.pkl"%experiment_number,'rb'))

	# Set the constructor
	myObject = Autoencoder(hidden_size=hidden_size)
	...
	filename = "Models/AutoE/trainset%d.pkl"%(experiment_number)
